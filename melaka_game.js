...
let customApiKey = "worker"; let gData = []; let cIdx = -1; let aiCnt = 0;
...
async function detectModels() {
    ...
    try {
        ...
        select.innerHTML = '';
        ['models/gemini-3.5-flash-lite', 'models/gemini-2.5-flash', 'models/gemini-2.0-flash', 'models/gemini-1.5-flash'].forEach(m => {
            const opt = document.createElement('option');
            opt.value = m;
            opt.text = m;
            select.appendChild(opt);
